...
def get_other_chats(id: int):
    with sq.connect(db_path) as con:
        ...
        cur.execute(f"""SELECT DISTINCT 
            ...
            FROM messages
            WHERE id_sender = ? OR receiver_id = ?
            ORDER BY rowid DESC
            LIMIT 7;""", (id, id))
        all_id = cur.fetchall()
        list_infos = []
        ...
        for i in all_id:
            if i[0] == id:
                cur.execute(
                    "SELECT name, avatar from users WHERE rowid = ?", (i[1],))
                list_infos.insert(0, [i[1], cur.fetchone()])
            else:
                cur.execute(
                    "SELECT name, avatar from users WHERE rowid = ?", (i[0],))
                list_infos.insert(0, [i[0], cur.fetchone()])

        list_infos.reverse()  # чтобы от последних чатов к старым
...
def get_subscribers(id: int):
    with sq.connect(db_path) as con:
        cur = con.cursor()
        cur.execute(
            "SELECT author from subscribers WHERE subscriber = ? LIMIT 7", (id,))
        ...
        all_subs.reverse()  # от новых подписок к старым
