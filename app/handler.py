...
@router.get("/load-feed")
async def load_feed(request: Request, offset: int = Query(..., gt=-1),
    ...
    try:
        ...
        with sq.connect(db_path) as con:
            ...
            cur.execute("""
                SELECT
                    ...
                    CASE WHEN EXISTS(
                        SELECT 1 FROM subscribers s WHERE s.subscriber = ? AND s.author = p.who
                    ) THEN 1 ELSE 0 END AS is_sub
                FROM posts p
                JOIN users u ON u.ROWID = p.who
                LEFT JOIN like l ON l.whom = p.post_id AND l.who = ?
                ORDER BY is_sub DESC, p.timestamp DESC, p.post_id DESC
                LIMIT ? OFFSET ?
        ...
        return JSONResponse(content={"posts": feed_posts})
...
async def login(request: Request):
    ...
    with sq.connect(db_path) as con:
        cur = con.cursor()
        cur.execute(f"SELECT * FROM users WHERE login = ? LIMIT 1",
                    (login.lower(),))
        test_user = cur.fetchone()
        # успешный вход
        if (test_user) and (check_password(test_user[2], password)):
...
async def register(request: Request):
    ...
    with sq.connect(db_path) as con:  # проверка, если логин уже есть в БД
        cur = con.cursor()
        cur.execute(
            f"SELECT login FROM users WHERE login = ? LIMIT 1", (login,))
    ...
    cur.execute("INSERT INTO users (name, login, password) VALUES (?, ?, ?)",
                (name, login.lower(), hashed_password))
    rowid = cur.lastrowid
    cur.execute(
        "INSERT INTO media (posts, sub, likes, followers) VALUES (0, 0, 0, 0)")
    con.commit()
    request.session["user_logged"] = True
    request.session["user_data"] = {
        'name': name,
